...
public class LambdasLab2 {
	...
	void PredicateTest() {
		...
		Predicate<String> test3 = s -> s.endsWith("a");
		Assertions.assertEquals(2,filterStrings(new String[] {"Cori","Jason","Pedro","Maria","Roberto","Raul","Petra"},test2.and(test3)).size());
	...
	void ConsumerTest() {
		...
		Consumer<String> consumer2 = s -> s.toUpperCase();
		consumeStrings(new String[] {"Cori","Jason","Pedro","Maria","Roberto","Raul","Petra"}, consumer2.andThen(consumer));
	...
	//TODO: complete mapPeopleToAges to convert a list of People into its age
	 List<Integer> mapPeopleToAges(List<People> peopleList, ToIntFunction<People> function) {
		 List<Integer> peopleAges = new ArrayList<Integer>();
		 for(People people:peopleList)
		 {
			 peopleAges.add(function.applyAsInt(people));
		 }
	...
	void FunctionTest() {
		...
		ToDoubleFunction<List<Integer>> mapper3 = list -> {
			double sum = 0;
			for(Integer i : list)
			{
				sum += i;
		...
		Assertions.assertEquals(mapper3.applyAsDouble(peopleAges), avgAge);
	}
